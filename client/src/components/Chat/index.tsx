...
import IChannel from '../../interfaces/components/Channel'
import { Context } from '../../main'
import { observer } from 'mobx-react-lite'
import ISocket from '../../interfaces/socket/Socket'

const Chat:FC<IChannel & ISocket> = (props):JSX.Element => {
  const {store} = useContext(Context)
  const [message, setMessage] = useState("")
  const [members, setMembers] = useState(false)
  const server = store.user.guilds.filter(e => e._id == props.guild)
  const category = server[0].category.filter(e => e._id == props.category)
  const channel = category[0].channels.filter(e => e._id == props.channel)

  const sendMessage = () => {
    props.socket.emit("send_message", {message, guild: server[0].name, category: category[0].name, channel: channel[0].name, email: store.user.email})
  }
    ...
    <div className={styles.chat}>
      <div className={styles.top_btns}>
        ...
        <div className={styles.top_btns_buttons}>
          ...
          <BiVolumeMute />
          <AiOutlinePaperClip />
          <HiUsers onClick={() => setMembers(!members)} />
        ...
        <div className={styles.messages_container}>
          <div className={styles.messages}>
            <div className={styles.greeting}>
              <h1>Добро пожаловать в канал # general</h1>
              <p>Это начало канала # general</p>
              <p>Настроить канал</p>
            </div>
            {channel[0].messages.map((e, index) => (
